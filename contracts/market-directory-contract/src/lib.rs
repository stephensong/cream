...
mod contract_impl {
    use cream_common::market::{MarketDirectoryState, MarketDirectorySummary};
    use freenet_stdlib::prelude::*;

    pub struct Contract;

    fn merge_validated(
        directory: &mut MarketDirectoryState,
        bytes: &[u8],
    ) -> Result<(), ContractError> {
        if bytes.is_empty() {
            return Ok(());
        }
        let update: MarketDirectoryState =
    ...
    impl ContractInterface for Contract {
        fn validate_state(
            ...
            let directory: MarketDirectoryState =
                serde_json::from_slice(bytes).map_err(|e| ContractError::Deser(e.to_string()))?;
        ...
        fn summarize_state(
            ...
            let directory: MarketDirectoryState = serde_json::from_slice(state.as_ref())
                .map_err(|e| ContractError::Deser(e.to_string()))?;
        ...
        fn get_state_delta(
            ...
            let summary: MarketDirectorySummary = if summary.is_empty() {
                MarketDirectorySummary::default()
            } else {
